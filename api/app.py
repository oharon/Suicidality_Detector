...
if st.button("Analyze"):
    ...
    st.markdown(f"<h2 style='text-align: center; color: black;'>{prediction['max_val']}</h2>", unsafe_allow_html=True)

    st.markdown("<br>", unsafe_allow_html=True)  # space
    st.markdown(f"<div style='font-size: 16px; color: black; '>{prediction['explanation']}</div>", unsafe_allow_html=True)

    if prediction['recommendation']:
        if prediction['max_val'] in ['Ideation', 'Indicator']:
            st.markdown("<br>", unsafe_allow_html=True)  # space
            st.markdown(f"<div style='font-size: 20px; color: black;'>{prediction['recommendation']}</div>", unsafe_allow_html=True)
        else:
            st.markdown("<br>", unsafe_allow_html=True)  # space
            st.markdown(f"<div style='font-size: 20px; color: red;'>{prediction['recommendation']}</div>", unsafe_allow_html=True)
